...
* **📅 스마트 시간표 렌더링**: 7×12 그리드 시스템을 지원하며, `rowspan` 자동 계산을 통해 연강 수업을 겹침 없이 매끄럽게 표시합니다.
* **🚨 지능형 알림 메모**: 독자적인 `Priority-DDL` 가중치 알고리즘을 적용하여, 기한이 임박하거나 지난 작업은 딥 레드(Deep Red) 컬러로 강조 경고합니다.
* **⏳ 몰입형 포모도로 타이머**: 사용자 정의 시간이 가능한 카운트다운 시스템으로, 고강도 시험 대비 및 집중 학습 환경에 최적화되어 있습니다.
* **🌍 글로벌 언어 지원**: 4개 국어 매핑 엔진을 내장하여, 데이터 손실 없이 UI 전체 텍스트를 실시간으로 전환할 수 있습니다.
* **🔐 개인정보 보호**: 모든 데이터는 `campus_data.json`을 통해 로컬에만 저장됩니다. 클라우드 업로드 없이 사용자의 프라이버시를 완벽하게 보장합니다.
